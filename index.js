...
function listOfNames(array) {
  let arrayCopy = array.map(({name}) => name);
  return arrayCopy;
}
...
function addArtist(array, name, years, genre, nationality, bio) {
  let arrayCopy = [...array];
  const newObject = {name: name, years: years, genre: genre, nationality: nationality, bio: bio};
...
let getBio = [];
  for ( let i = 0; i < array.length; i ++ ){
    ...
    getBio.push(array[i].bio);
  }
}
return getBio.toString()
}
